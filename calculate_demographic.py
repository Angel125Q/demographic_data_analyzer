...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round(len(num_min_worker[num_min_workers.salary == ">50k "])/ len(num_min_workers) *100, 1)

    # What country has the highest percentage of people that earn >50K?
    num_country = df["native-country"].value_counts()
    country_rich_count = df[df["salary"] == '>50K']["native-country"].value_counts()
    highest_earning_country = (country_rich_count / num_country * 100).idxmax()
    highest_earning_country_percentage = round((country_rich_count/ num_country * 100).max(), 1)

    # Identify the most popular occupation for those who earn >50K in India.
    india_people = df[(df["native-country"] == "India") & (df["salary"] == ">50K")]
    occupation_counts = india_people["occupation"].value_counts()
